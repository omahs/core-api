...
describe Givings::Payment::CreateOrder do
  describe '.call' do
    subject(:command) { described_class.call(order_type, args) }

    let(:order_types_module) { Givings::Payment::OrderTypes }

    let(:user) { create(:user) }
    let(:customer) { build(:customer) }

    context 'using a CreditCard payment' do
      let(:order_type) { order_types_module::CreditCard }
    ...
    context 'using a Crypto payment' do
      let(:order_type) { order_types_module::Cryptocurrency }
