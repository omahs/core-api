...
class ContributionBalance < ApplicationRecord
  ...
  scope :total_tickets_balance_from_big_donors, -> { joins(contribution: :person_payment)
    .where(person_payments: { payer_type: "BigDonor" })
    .sum(:tickets_balance_cents) }

  scope :total_tickets_balance_from_unique_donors, -> { joins(contribution: :person_payment)
