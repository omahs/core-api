...
RSpec.describe GivingServices::Payment::Orchestrator, type: :service do
  subject(:service_call) { described_class.new(payload: payload).call }
  ...
  describe '#call' do
    ...
    let!(:gateway_instance) { mock_instance(klass: gateway::PaymentProcessor, mock_methods: [:purchase, :subscribe, :unsubscribe]) }
    
    context 'when the Orchestrator is instanced' do
      let(:operation) { :purchase }
      let!(:factory_instance) { mock_instance(klass: GivingServices::Payment::GatewayFactory, mock_methods: [:call]) }

      it 'calls GatewayFactory and returns a gateway module' do
        expect(factory_instance).to receive(:call).and_return(gateway)
    ...
    context 'when is a purchase operation' do
      ...
      it 'calls PaymentProcessor#purchase method' do
        expect(gateway_instance).to receive(operation)
